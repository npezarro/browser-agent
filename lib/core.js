...
function shouldRouteToExtension(action, extLastHeartbeat, extTtl, now) {
  now = now || Date.now();
  const EXT_TAB_ACTIONS = new Set(["openTab", "openTabBackground", "closeTab", "focusTab", "queryTabs", "createTab", "captureTab", "captureAdvanced", "cdpType", "cdpClick", "cdpEval", "cdpKeys", "cdpNetworkCapture", "extractVirtual", "reloadExtension"]);
